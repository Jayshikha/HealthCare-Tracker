...
export class UserFormComponent implements OnInit {
  ...
  constructor(private userDataService: UserDataService) {
    ...
    // console.log("harshit",dropdownfinal)
  }
  ...
  adduser(event: any): void {
    ...
    const workoutType = (document.getElementById("choices") as HTMLInputElement).value;
    // const workoutTime = (document.getElementById("workoutTime") as HTMLInputElement).value;
    const workoutTime = parseInt((document.getElementById("workoutTime") as HTMLInputElement).value, 10);
    ...
    if (!existingUser) {
      ...
      const userid = UserData.length > 0 ? UserData[UserData.length - 1].id + 1 : 1;
      const newuser: User = { id: userid, name: username, workouts: workouts };
