...
export class WorkoutListComponent implements OnInit {
  ...
  //   for(var i =0 ;i<user.workouts.length;i++){
  //     totalMinutes += user.workouts[i].minutes;
  //   }
  //   // console.log("totalmintureharshti",totalMinutes)
  //   return totalMinutes;
  // }
  getTotalMinutes(userId: number): number {
  ...
  onChange(event: Event) {
   ...
   // alert(inputValue)
    ...
    this.users = this.userDataService.getUsers().filter(user =>
  ...
  fetchUsers(): void {
    this.userDataService.getPage(this.currentPage, this.itemsPerPage).subscribe({
      next: users => {
        this.users = users;
        this.totalPages = Math.ceil(this.userDataService.getTotalUsersCount() / this.itemsPerPage);
      },
      error: err => {
